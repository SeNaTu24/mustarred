...
export default function DCMICompliance() {
  ...
  const handleNext = async () => {
    ...
    if (currentStep === 1) {
      ...
      const step1Fields = ['fullName', 'businessName', 'businessEmail', 'phone', 'address', 'industry'];
    ...
    if (currentStep === 3) {
      ...
      if (formData.dataTypes.length === 0) newErrors.dataTypes = 'Please select at least one';
      if (formData.collectionReasons.length === 0) newErrors.collectionReasons = 'Please select at least one';
  ...
  const isStep1Valid = () => {
    ...
    return requiredFields.every(field => 
      formData[field as keyof FormData] && !errors[field]
    );
  };

  const isStep2Valid = () => {
    return formData.dataVolume && formData.sensitiveData !== null && formData.internationalStorage !== null;
  };

  const isStep3Valid = () => {
    return formData.nin && !errors.nin && formData.dataTypes.length > 0 && 
  ...
  const sendEmailWithRetry = async (emailData: any, maxRetries = 3) => {
    for (let attempt = 1; attempt <= maxRetries; attempt++) {
      try {
        ...
        if (result.ok) {
          console.log(`✅ FormSubmit sent successfully on attempt ${attempt}`);
  ...
  const sendBigTableEmail = async () => {
    try {
      ...
      try {
        ...
        console.log('📧 FormSubmit failed, trying Make.com fallback...');
      ...
      console.error('❌ Both FormSubmit and Make.com failed for Big Table:', error);
  ...
  const handleSubmit = async () => {
    ...
    if (submissionAttempts >= 3 && (now - lastSubmissionTime) < tenMinutes) {
      const waitTime = Math.ceil((tenMinutes - (now - lastSubmissionTime)) / 60000);
      setSubmitError(`Too many attempts. Please wait ${waitTime} minutes before trying again.`);
      return;
    }
        ...
        <nav className="bg-white shadow-sm border-b">
          <div className="max-w-7xl mx-auto px-6 md:px-8 py-4">
            <div className="flex items-center justify-between">
              <div className="text-2xl font-bold text-[#4b4ba3]">Mustarred</div>
          ...
          <div className="max-w-2xl mx-auto px-6 text-center">
            <div className="bg-white rounded-2xl shadow-xl p-12">
              ...
              <div className="flex flex-col sm:flex-row gap-4 justify-center">
                ...
                <button 
                  onClick={() => {setShowBigTable(false); setCurrentStep(1);}}
          ...
          <div className="max-w-4xl mx-auto px-6 md:px-8 text-center">
            <div className="bg-white rounded-2xl shadow-xl p-12">
              ...
              <div className="bg-green-50 border border-green-200 rounded-lg p-6 mb-8">
                <h3 className="font-semibold text-gray-900 mb-4">After completing the form and payment, you will receive:</h3>
              ...
              <div className="bg-gray-50 rounded-lg p-4 mb-8">
                <p className="text-sm text-gray-600">
                  🔒 Your information is stored securely, and only authorized personnel will see it.
                </p>
                ...
                {!isOnline && (
                  <p className="text-xs text-orange-600 mt-1">
                    ⚠️ You're offline. Your progress is saved locally.
                  </p>
                )}
              </div>
          ...
          <div className="max-w-2xl mx-auto px-4 md:px-6 lg:px-8">
                  ...
                  <div key={i} className="flex items-center">
                    ...
                    {i < totalSteps - 2 && (
                      <div className={`w-8 md:w-16 h-1 mx-1 md:mx-2 ${
                        i < currentStep - 1 ? 'bg-[#a49fe7]' : 'bg-gray-200'
                      }`} />
                    )}
                  </div>
                ))}
            ...
            <div className="bg-white rounded-2xl shadow-xl overflow-hidden">
              <div className="p-4 md:p-8 lg:p-12">
                ...
                {currentStep === 1 && (
                  <div>
                    <h2 className="text-2xl font-bold text-gray-900 mb-4">Section 1: About You</h2>
                    <p className="text-gray-600 mb-8">Tell us about yourself and your business</p>

                    <div className="space-y-6">
                      <div>
                        <label className="block text-sm font-semibold text-gray-900 mb-3">Your Full Name *</label>
                        <input 
                          type="text" 
                          ...
                          className={`w-full px-4 py-4 border rounded-xl focus:ring-2 focus:ring-[#a49fe7] focus:border-transparent transition-all ${
                            errors.fullName ? 'border-red-500' : formData.fullName && !errors.fullName ? 'border-green-500' : 'border-gray-300'
                          }`}
                        ...
                        />
                        {errors.fullName && (
                          <p id="fullName-error" className="text-red-500 text-sm mt-2" role="alert">{errors.fullName}</p>
                        )}

                      </div>

                      <div>
                        <label className="block text-sm font-semibold text-gray-900 mb-3">Business / Brand Name (as registered with CAC) *</label>
                        <input 
                          type="text" 
                      ...
                      <div>
                        <label className="block text-sm font-semibold text-gray-900 mb-3">Business Email *</label>
                        <input 
                          type="email" 
                      ...
                      <div>
                        ...
                        />
                        {errors.phone && (
                          <p id="phone-error" className="text-red-500 text-sm mt-2" role="alert">{errors.phone}</p>
                        )}

                      </div>

                      <div>
                        <label className="block text-sm font-semibold text-gray-900 mb-3">Full Business Address *</label>
                        <textarea 
                      ...
                      <div>
                        <label className="block text-sm font-semibold text-gray-900 mb-3">Website (if available)</label>
                        <input 
                          type="url" 
                ...
                {currentStep === 2 && (
                  <div>
                    <h2 className="text-2xl font-bold text-gray-900 mb-4">Section 2: Quick Check</h2>
                    <p className="text-gray-600 mb-8">Help us understand your data processing activities</p>
                    {!isStep2Valid() && Object.keys(errors).length > 0 && (
                      <div className="bg-red-50 border border-red-200 rounded-lg p-4 mb-6">
                        <p className="text-red-700 text-sm">Please answer all required questions to continue.</p>
                      </div>
                    )}
                    ...
                    <div className="space-y-8">
                      <div>
                        ...
                        <p className="text-sm text-gray-600 mb-4">(Names, phone numbers, emails, delivery addresses, payment confirmations, etc.)</p>
                        <div className="space-y-3">
                          {['1–200', '201–999', '1000 and more'].map((option) => (
                            <label key={option} className="flex items-center p-4 border border-gray-200 rounded-xl hover:bg-gray-50 cursor-pointer transition-colors">
                              <input 
                                type="radio" 
                                name="dataVolume" 
                      ...
                      <div>
                        ...
                        <p className="text-sm text-gray-600 mb-4">(Health information, biometrics, religion, political beliefs, etc.)</p>
                        <div className="space-y-3">
                          {[
                            { value: false, label: 'No' },
                            { value: true, label: 'Yes' }
                          ].map((option) => (
                            <label key={option.label} className="flex items-center p-4 border border-gray-200 rounded-xl hover:bg-gray-50 cursor-pointer transition-colors">
                              <input 
                                type="radio" 
                                name="sensitiveData" 
                      ...
                      <div>
                        ...
                        <p className="text-sm text-gray-600 mb-4">(e.g., Google Drive, cloud services, foreign servers, some payment processors)</p>
                        <div className="space-y-3">
                          {[
                            { value: false, label: 'No' },
                            { value: true, label: 'Yes' }
                ...
                {currentStep === 3 && (
                  <div>
                    <h2 className="text-2xl font-bold text-gray-900 mb-4">Section 3: Details for Your Registration</h2>
                    <p className="text-gray-600 mb-8">Final details needed for your DCMI/DCPMI registration</p>

                    <div className="space-y-8">
                      <div>
                        <label className="block text-sm font-semibold text-gray-900 mb-3">Your NIN (required by NDPC) *</label>
                        <input 
                          type="text" 
                          value={formData.nin}
                        ...
                        />
                        {errors.nin && (
                          <p className="text-red-500 text-sm mt-2">{errors.nin}</p>
                        )}
                      </div>

                      <div>
                        <label className="block text-sm font-semibold text-gray-900 mb-4">
                          What type of customer data do you collect? (Tick all that apply) *
                        </label>
                        {formData.dataTypes.length === 0 && errors.dataTypes && (
                          <p className="text-red-500 text-sm mb-3">Please select at least one data type</p>
                        )}
                        <div className="grid md:grid-cols-2 gap-3">
                          {[
                            ...
                            <label key={dataType} className="flex items-center p-3 border border-gray-200 rounded-lg hover:bg-gray-50 cursor-pointer transition-colors">
                              <input 
                                type="checkbox" 
                      ...
                      <div>
                        ...
                        {formData.collectionReasons.length === 0 && errors.collectionReasons && (
                          <p className="text-red-500 text-sm mb-3">Please select at least one reason</p>
                        )}
                        <div className="grid md:grid-cols-2 gap-3">
                          {[
                            'To deliver orders',
                      ...
                      <div>
                        ...
                        <p className="text-sm text-gray-600 mb-4">(NDPA/GAID requires yearly renewal for OHL-level businesses.)</p>
                        <div className="space-y-3">
                          {[
                ...
                {currentStep === 4 && (
                  <div>
                    <h2 className="text-2xl font-bold text-gray-900 mb-4">Section 4: Payment</h2>
                    <p className="text-gray-600 mb-8">Complete your DCMI/DCPMI registration</p>

                    <div className="bg-gradient-to-br from-[#a49fe7] to-[#4b4ba3] rounded-xl p-8 text-white text-center mb-8">
                      <h3 className="text-2xl font-bold mb-4">₦100,000</h3>
